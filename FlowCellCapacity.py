...
if len(fcs_tumor) > 1:
    fcs_tumor = sorted(fcs_tumor, reverse=True)
    percentfull = sum(fcs_tumor) / sum(fcsmax_tumor)
    original_fcdesign = fcs_tumor
    fcs_tumor = [round(i * percentfull) for i in fcs_tumor]
